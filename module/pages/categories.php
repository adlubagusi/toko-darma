<?php
$cLink  = $_GET['c'];
$dbData = mysqli_query($db,"select * from categories where link='$cLink'");
$vaCat  = mysqli_fetch_array($dbData);
$cIdCat = $vaCat['id'];
...
$dbProduct = mysqli_query($db,"select * from products where category='$cIdCat' order by $cOrderBy");
...
<div class="wrapper">
    <div class="title-head">
        <?php if($cTitleHead == ""){ ?>
            <h2 class="title">Kategori <?= $vaCat['name'] ?></h2>
        <?php }else{ ?>
            <h2 class="title">Kategori <?= $vaCat['name'] ?> > <?= $cTitleHead ?></h2>
        <?php } ?>
    </div>
    <div class="core">
        <div class="filter">
            <div class="filter-main">
                ...
                <div class="bodf">
                    ...
                    <a href="?c=<?= $cLink ?>&ob=latest">Latest</a>
                    <a href="?c=<?= $cLink ?>&ob=az">Alphabet A-Z</a>
                    <a href="?c=<?= $cLink ?>&ob=za">Alphabet Z-A</a>
                    <hr>
                    <p class="title">
                    Condition
                    </p>
                    <a href="?c=<?= $cLink ?>&condition=1">New</a>
                    <a href="?c=<?= $cLink ?>&condition=2">Second</a>
                    <hr>
